...
int main(int argc, char **argv)
{
  ...
  // Initialization: read RBTree from a binary file
  ...
  for (int k = 1; k != (total_iterations + 1); ++k) {
    ...
    if (rt_SrchIter == rt.end()) {
      ...
      mcsim_log_begin();           
      // log the to-be-insert position
      undolog.insert(Int_Pair(rt_SrchIter->first, rt_SrchIter->second));
      // log the new entry to be inserted
      //mcsim_skip_instrs_begin();
      //redolog.insert(Int_Pair(search_key, v));
      //mcsim_skip_instrs_end();
      mcsim_mem_fence();
      mcsim_log_end();
      mcsim_mem_fence();
      ...
      mcsim_log_begin();
      // log the to-be-remove entry
      undolog.insert(Int_Pair(search_key, rt_SrchIter->second));
      //mcsim_skip_instrs_begin();
      //redolog.insert(Int_Pair(search_key, rt_SrchIter->second));
      //mcsim_skip_instrs_end();
      mcsim_mem_fence();
      mcsim_log_end();
      mcsim_mem_fence();
  ...
  mcsim_skip_instrs_begin();
  std::cout << "dummy: undolog.size= " << undolog.size() << std::endl;
  std::cout << "dummy: redolog.size= " << redolog.size() << std::endl;
  mcsim_skip_instrs_end();
